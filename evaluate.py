...
if __name__ == '__main__':
    ...
    for i in tqdm(range(n_runs)):
        ...
        # ---- Tukey's transform
        beta = 0.5
        support_data = np.power(support_data[:, ] + 1e-6,beta)
        query_data = np.power(query_data[:, ] + 1e-6,beta)
        ...
        # ---- train classifier
        classifier = LogisticRegression(C=15, multi_class='auto', solver='lbfgs', max_iter=1000).fit(X=X_aug, y=Y_aug)
